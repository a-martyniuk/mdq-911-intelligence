...
export default function SectionMalvinasHotspots({ incidents = [] }: SectionMalvinasHotspotsProps) {
  ...
  // Top hostile node dynamically derived from Malvinas chronic nodes
  const maxHostilityNode = useMemo(() => {
    return [...CHRONIC_HOTSPOTS_MALVINAS].sort((a, b) => b.pctArmed - a.pctArmed)[0];
  }, []);
  ...
  // Selected Node for modal dossier
    ...
    filtered.forEach((inc) => {
      ...
      if (!map[key]) {
        map[key] = {
          ...
          count: 0,
          armedCount: 0,
          barrio: inc.barrio || "Malvinas Argentinas",
          lat: inc.lat,
          lng: inc.lng,
          substances: {},
          slots: {},
  ...
  const top10TotalCalls = useMemo(() => {
    return CHRONIC_HOTSPOTS_MALVINAS.reduce((acc, n) => acc + n.totalIncidents, 0);
  }, []);

  const top10ArmedCalls = useMemo(() => {
    return CHRONIC_HOTSPOTS_MALVINAS.reduce((acc, n) => acc + n.armedIncidents, 0);
  }, []);

  const top10Bunkers = useMemo(() => {
    return CHRONIC_HOTSPOTS_MALVINAS.reduce((acc, n) => acc + n.bunkersCount, 0);
  }, []);
  ...
  useEffect(() => {
    ...
    import("leaflet").then((LModule) => {
      ...
      CHRONIC_HOTSPOTS_MALVINAS.forEach((node) => {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "1rem", marginTop: "1.25rem" }}>
          ...
          <div style={{ background: "rgba(0,0,0,0.25)", padding: "0.85rem", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div style={{ fontSize: "1.05rem", fontWeight: 900, color: "#dc2626", marginTop: "2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={maxHostilityNode?.name}>
              {maxHostilityNode?.shortName || "Py & Hiroshima"}
            </div>
      ...
      {activeTab === "nodes" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: "1rem" }}>
            {CHRONIC_HOTSPOTS_MALVINAS.map((node) => {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(450px, 1fr))", gap: "1.5rem" }}>
          <div className="card">
            ...
            <p style={{ fontSize: "0.82rem", color: "#9ca3af", lineHeight: 1.4 }}>
              En Malvinas Argentinas se comprueba la regla empírica del 80/20 del delito urbano: una porción diminuta del territorio absorbe la gran mayoría de los recursos del 911 y de violencia armada.
            </p>

            <div style={{ marginTop: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              {CHRONIC_HOTSPOTS_MALVINAS.map((n, i) => {
                const cumPct = (CHRONIC_HOTSPOTS_MALVINAS.slice(0, i + 1).reduce((a, b) => a + b.totalIncidents, 0) / (totalCalls || 1) * 100).toFixed(1);
                return (
                  <div key={n.id}>
                    ...
                    <div style={{ width: "100%", height: "8px", background: "#1f2937", borderRadius: "4px", overflow: "hidden" }}>
                      <div style={{ width: `${(n.totalIncidents / (CHRONIC_HOTSPOTS_MALVINAS[0].totalIncidents || 1)) * 100}%`, height: "100%", background: "#ef4444" }} />
                    </div>
                  </div>
                );
              })}

...
export default function SectionMalvinasGraph({ incidents = [] }: SectionMalvinasGraphProps) {
  ...
  // -------------------------------------------------------------
  // 1. DYNAMIC GRAPH CONSTRUCTION FROM 1,471 INCIDENTS
  // -------------------------------------------------------------
  const { allNodes, allEdges } = useMemo(() => {
    ...
    incidents.forEach((inc) => {
      ...
      if (addrClean && !["Malvinas Argentinas", "OTRA", ".", "OTRO", ":", "....", "NAN"].includes(addrClean.toUpperCase())) {
    ...
    Object.entries(bunkerMap)
      ...
      .forEach(([addr, data]) => {
        ...
        bunkerNodes.push({
          ...
          cliqueId: data.clique || "general",
          address: addr,
          barrio: data.barrio || "Malvinas Argentinas",
          coords: data.coords,
          incidents: data.calls
        });
      });
    ...
    <div className="animate-enter" style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.85rem" }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                ...
                <span style={{ fontSize: "11px", background: "rgba(245, 158, 11, 0.12)", color: "#fcd34d", border: "1px solid rgba(245, 158, 11, 0.25)", padding: "2px 8px", borderRadius: "var(--radius-xs)", fontWeight: 700, fontFamily: "var(--font-mono)" }}>
                  EJE 2 FORENSE · MALVINAS ARGENTINAS
                </span>
              </div>
              <p style={{ margin: "0.25rem 0 0", fontSize: "13.5px", color: "var(--text-muted)" }}>
                Modelado topológico de co-ocurrencia: Sospechosos/Alias &harr; Puntos de Venta/Búnkers &harr; Sustancias &harr; Conflictividad Armada (1.471 llamados 911).
              </p>
            </div>
          </div>
      ...
      <div className="card" style={{ padding: "1rem" }}>
        <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", marginBottom: "0.6rem", display: "flex", alignItems: "center", gap: "0.4rem" }}>
          <Target size={14} /> Focos Criminales / Bandas Territoriales Detectadas en Malvinas Argentinas:
        </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.5rem" }}>
              <div>
                ...
                <div style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                  {activeSelectedNode.address || activeSelectedNode.barrio || "Partido de Malvinas Argentinas"}
                </div>
              </div>
                ...
                {activeSelectedNode.incidents.slice(0, 15).map((inc, i) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: "0.75rem", color: "var(--text-primary)", marginBottom: "0.35rem" }}>
                      📍 <strong>{inc.direccion || inc.Dirección || "Malvinas Argentinas"}</strong>
